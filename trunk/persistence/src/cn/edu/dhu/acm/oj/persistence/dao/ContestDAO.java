...
public class ContestDAO extends BaseHibernateDAO {
    ...
    public List<ContestBean> findContestInRange(int first, int max){
        try {
            Session session=getSession();
            Transaction tx = session.beginTransaction();
            Query query=session.createQuery("from ContestBean where contest_id>0 order by contest_id desc");
    ...
    public List<ContestBean> findContestByStatus(int status){
        try {
            Session session=getSession();
            Transaction tx = session.beginTransaction();
            Query query=session.createQuery("from ContestBean where contest_id>0 and status=" + status);
            List<ContestBean> rs=query.list();
            tx.commit();
            return rs;
        } catch(Exception e) {
